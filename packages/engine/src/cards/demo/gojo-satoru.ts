  ...
  abilities: [
    {
      ...
      name: "L'Infini",
      kind: 'passive',
      description: "Gojo bénéficie de l'effet Esquive",
    ...
    },
    {
      ...
      kind: 'active',
      description: "Inflige Stun au personnage actif adverse pendant son prochain tour utilisable qu'une fois",
      usesPerGame: 1,
      async execute(ctx) {
        const target = ctx.getActive(ctx.opponentId);
        if (!target) return;
