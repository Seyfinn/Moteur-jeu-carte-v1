...
## Reprise de partie, revanche et prompts abandonnés

- **Revanche** : à la fin d'une partie, l'écran de résultat propose de rejouer *dans le même salon*. Il faut une demande de chaque camp (message `rematch`) : le serveur annonce la première à l'adversaire (`rematch-requested`), et quand les deux sont là il crée une nouvelle `Match` avec les mêmes noms et les mêmes decks, diffusée comme n'importe quel état. Côté client, le plateau est monté avec l'identifiant de partie en clé, donc tout ce qu'il garde en mémoire d'un tour à l'autre (roue d'initiative déjà jouée, journal déjà animé) repart de zéro.
